...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Rebecca L. Taylor" };
...
const char *CL_FILE_NAME   = { "two_sphere_particles.cl" };
...
Animate( )
{
	...
	if( ShowPerformance )
	{
		...
		// record peak performance in Mega/Sec
		float currPerformance = (float)NUM_PARTICLES / ElapsedTime / 1000000.;
		if (currPerformance > peakPerformance)
			peakPerformance = currPerformance;
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		...
		glColor3f( .9f, .9f, 0. );
		glPushMatrix( );
			glTranslatef( -100.f, -800.f, 0.f );	// sphere center
			glutWireSphere( 600.f, 100, 100 );		// sphere radius, slices, stacks
		glPopMatrix( );
		// Sphere 2
		glColor3f(.9f, .9f, 0.);
		glPushMatrix();
		glTranslatef(500.f, 80.f, 0.f);				// sphere center
		glutWireSphere(200.f, 100, 100);			// sphere radius, slices, stacks
		glPopMatrix();
	glEndList( );
...
ResetParticles( )
{
	...
	for( int i = 0; i < NUM_PARTICLES; i++ )
	{
		...
		// set initial particle color to pink
		colors[i].r = 0.804;
		colors[i].g = 0.361;
		colors[i].b = 0.361;
		colors[i].a = 1.;
...
{
	// record performance
	printf("Number of particles: %d, Peak performance: %8.3lf MegaParticles/Sec\n", NUM_PARTICLES, peakPerformance);

	FILE* outputResults = fopen("particle_results.csv", "a");
	if (outputResults == NULL)
	{
		printf("Error: no output file\n");
		exit(1);
	}
	fprintf(outputResults, "%d, %.2lf\n", NUM_PARTICLES, peakPerformance);
